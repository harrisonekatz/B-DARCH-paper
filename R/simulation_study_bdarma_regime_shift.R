...
for(i in 1:n_sim){
  ...
  while(!valid_simulation) {
    ...
    tryCatch({
      ...
      A_shock <- matrix(runif(16, -.75, .75), nrow = 4)
      beta_y <- rnorm(5, mean = 0.2, sd = 0.03)
      beta_y <- beta_y / sum(beta_y)
      beta <- alr(beta_y)


      beta_y_shock <- rnorm(5, mean = 0.2, sd = 0.03)
      beta_y_shock <- beta_y_shock / sum(beta_y_shock)
      beta_shock <- alr(beta_y_shock)

      phi_beta <- runif(1, min = 6, max = 7.5)
      phi_beta_shock <- runif(1, min = 6, max = 7.5)
      phi_trend <- 0
      alpha <- 0.8
      gamma <- -.95
      ...
      eta[1,] <- alr(y[1,])
      missreported <- floor(runif(1,min=10,max=50))
      missreported <- seq(missreported,missreported+9)

      # Generate a single sigma for the whole simulation, with a complex covariance structure
      sigma <- runif(1, 0.05, 0.5)
      sigma_shock <- runif(1, 0.05, 0.5)
      cov_matrix <- generate_cov_matrix(4)
      cov_matrix_shock <- generate_cov_matrix(4) # Generate a complex covariance matrix

      # Start simulation loop for tVARMA
      for (k in 2:(min(missreported)-1)) {
        # Use the single sigma with the more complex covariance matrix
        eta[k,] <- beta + (A %*% (alr(y[k-1,]) - beta))
        phi[k] <- phi_beta
        exp_phi <- exp(phi[k])
        alpha_y <- exp_phi * alrinv(eta[k,])

        y_proposed <- rdirichlet(1, alpha_y)
      ...
      for (k in (min(missreported):max(missreported))) {
        # Use the single sigma with the more complex covariance matrix
        eta[k,] <- beta_shock + (A_shock %*% (alr(y[k-1,]) - beta_shock))
        phi[k] <- phi_beta_shock
        exp_phi <- exp(phi[k])
        alpha_y <- exp_phi * alrinv(eta[k,])
      ...
      for (k in (max(missreported)+1):total_obs) {
        eta[k,] <- beta + (A %*% (alr(y[k-1,]) - beta))
        phi[k] <- phi_beta
        exp_phi <- exp(phi[k])
        alpha_y <- exp_phi * alrinv(eta[k,])
...
saveRDS(combined_metrics,file="summ_darma_regime_shift.RDS")
...
combined_pacf_darma_rs <- combined_pacf

saveRDS(combined_pacf_darma_rs,file="pacf_sim_darma_regime_shift.RDS")
