...
for(i in 1:n_sim){
  ...
  while(!valid_simulation) {
    ...
    beta <- alr(beta_y)
      ...
      phi_beta <- c(rep(phi_beta, (total_obs)))
      phi_trend <- 0
      alpha <- 0
      gamma <- 0
      phi <- matrix(data = NA, nrow = (total_obs))
      phi[1,] <- phi_beta[1] + phi_trend
      ...
      for (k in 2:(total_obs)) {
        ...
        phi[k] <- phi_beta[k] + alpha * (phi[k-1] - phi_beta[k-1]) + gamma * sum(alr(y[k-1,]) - eta[k-1,])^2
  ...
  matplot(y[1:total_obs,], type = "l", lty = 1, pch = 19, col = 1:5, xlab = "Index", ylab = "Value", main = "Plot of Matrix Columns")
...
saveRDS(combined_metrics,file="summ_darma_shock.RDS")
...
combined_pacf_darma_shock <- combined_pacf

saveRDS(combined_pacf_darma_shock,file="pacf_sim_darma_shock.RDS")
