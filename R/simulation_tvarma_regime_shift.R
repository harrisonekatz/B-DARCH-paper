  ...
  while(!valid_simulation) {
    ...
    tryCatch({
      ...
      for (k in (min(missreported):max(missreported))) {
        # Use the single sigma with the more complex covariance matrix
        eta[k,] <- mvrnorm(1, mu = beta_shock + (A_shock %*% (alr(y[k-1,]) - beta_shock)), Sigma = sigma_shock^2 * cov_matrix_shock)
...
i
saveRDS(combined_metrics,file="summ_varma_regime_shift.RDS")
...
saveRDS(combined_pacf_tvarma_shock,file="pacf_sim_tvarma_regime_shift.RDS")
